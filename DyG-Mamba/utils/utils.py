...
class NeighborSampler:
    ...
    def __init__(self, adj_list: list, sample_neighbor_strategy: str = 'uniform', 
                 time_scaling_factor: float = 0.0, seed: int = None, max_edge_id=10000, max_interact_times=100000):
        """
        Neighbor sampler.
        ...
        self.seed = seed
        self.max_node_id = len(adj_list)
        self.max_edge_id = max_edge_id
        self.max_interact_times = max_interact_times
        ...
        for node_idx, per_node_neighbors in enumerate(adj_list):
            ...
            sorted_per_node_neighbors = sorted(per_node_neighbors, key=lambda x: x[2]) # default ascending order
    ...
    def find_neighbors_before(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False, robust_rate=0):
        """
        ...
        """
        num_node = self.max_node_id
        num_edge = self.max_edge_id
        max_time = self.max_interact_times
        ...
        neighbor_ids = self.nodes_neighbor_ids[node_id][:i] # np.array
        neighbor_edge_ids = self.nodes_edge_ids[node_id][:i]
        neighbor_times = self.nodes_neighbor_times[node_id][:i]

        if robust_rate != 0:
            num_neighbors = len(neighbor_ids)
            num_noise_edges = int(robust_rate*num_neighbors)

            neighbor_ids = np.array(neighbor_ids)
            neighbor_edge_ids = np.array(neighbor_edge_ids)
            neighbor_times = np.array(neighbor_times)

            noise_positions = np.random.permutation(num_neighbors)[:num_noise_edges]
            neighbor_ids[noise_positions] = np.random.randint(1, num_node, num_noise_edges)
            neighbor_edge_ids[noise_positions] = np.random.randint(1, num_edge, num_noise_edges)
            neighbor_times[noise_positions] = np.random.randint(1, max_time, num_noise_edges)

            sorted_indices = np.argsort(neighbor_times)
            neighbor_ids = neighbor_ids[sorted_indices]
            neighbor_edge_ids = neighbor_edge_ids[sorted_indices]
            neighbor_times = neighbor_times[sorted_indices]
            # print("after: ", neighbor_ids, type(neighbor_ids))

        if return_sampled_probabilities:
            return neighbor_ids, neighbor_edge_ids, neighbor_times, \
                   self.nodes_neighbor_sampled_probabilities[node_id][:i]
        else:
            # According to robust rate to replace neighbors and timestamps
            return neighbor_ids, neighbor_edge_ids, neighbor_times, None

    def get_historical_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 20, robust_rate=0):
        """
    ...
    def get_all_first_hop_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray, robust_rate=0):
        """
...
def get_neighbor_sampler(data: Data, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
    ...
    max_edge_id = max(data.edge_ids)
    max_interact_times = max(data.node_interact_times)
    ...
    return NeighborSampler(adj_list=adj_list, sample_neighbor_strategy=sample_neighbor_strategy, 
                           time_scaling_factor=time_scaling_factor, seed=seed, max_edge_id=max_edge_id, 
                           max_interact_times=max_interact_times)
...
class NegativeEdgeSampler(object):
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, interact_times: np.ndarray = None, last_observed_time: float = None,
        ...
        if self.negative_sample_strategy == 'inductive':
            ...
            self.observed_edges = self.get_unique_edges_between_start_end_time(self.earliest_time, self.last_observed_time) # train edges + val edges
