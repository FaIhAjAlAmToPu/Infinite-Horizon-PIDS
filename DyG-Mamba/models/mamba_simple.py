...
class Mamba(nn.Module):
    def __init__(
        ...
        self.conv1d = nn.Conv1d(
            ...
            padding=d_conv - 1,
            **factory_kwargs,
        ) # L_out = (L_in + 2*padding - dilation*(kernel_size-1) - 1)/stride + 1 = L_in+d_conv-1
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None:  # Doesn't support outputting the states
            ...
            x_dbl = self.x_proj(rearrange(x, "b d l -> (b l) d"))  # (bl d) # [BL, dt_rank + d_state * 2]
            dt, B, C = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1) # [BL, dt_rank], [BL, dt_state], [BL, dt_state]
            dt = self.dt_proj.weight @ dt.t() # [dt_inner, dt_rank] [dt_rank, BL] -> [d_inner, BL]
            dt = rearrange(dt, "d (b l) -> b d l", l=seqlen) # [B, d_inner, L]
...
class Block(nn.Module):
    ...
    def forward(
        self, hidden_states: Tensor, residual: Optional[Tensor] = None, inference_params=None, dts=None
    ):
        ...
        hidden_states = self.mixer(hidden_states, inference_params=inference_params, dts=dts)
...
class MambaTimeDelta(nn.Module):
    def __init__(
        ...
        self.dt_generate_layer = nn.Linear(self.d_model, self.dt_rank, bias=False, **factory_kwargs)
    ...
    def forward(self, hidden_states, inference_params=None, dts=None):
        ...
        if self.time_mamba and dts != None:
            ...
            if self.in_proj.bias is not None:
                dts_xz = dts_xz + rearrange(self.in_proj.bias.to(dtype=dts_xz.dtype), "d -> d 1")
        ...
        dt = self.dt_generate_layer(rearrange(dts, "b l d -> (b l) d")) # [BL, D]
