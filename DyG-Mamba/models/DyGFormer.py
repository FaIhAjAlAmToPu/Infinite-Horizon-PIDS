...
class DyGFormer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler, 
                 time_feat_dim: int, channel_embedding_dim: int, experiment_settings: ExperimentSetting=None, 
                 patch_size: int = 1, num_layers: int = 2, num_heads: int = 2,
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray, robust_rate=0,
                                                 case_study=False):
        """
        ...
        for i, transformer in enumerate(self.transformers):
            ...
            if i == 0 and case_study:
                second_last_src_hidden_states = patches_data[:, : src_num_patches, :]
                second_last_dst_hidden_states = patches_data[:, src_num_patches: src_num_patches + dst_num_patches, :]
        ...
        if case_study:
            last_src_hidden_states = src_patches_data
            last_dst_hidden_states = dst_patches_data
        ...
        if case_study:
            pooled_src_embedding = src_patches_data
            pooled_dst_embedding = dst_patches_data
        ...
        if case_study:
            return src_node_embeddings, dst_node_embeddings, (pooled_src_embedding, pooled_dst_embedding), \
                (last_src_hidden_states, last_dst_hidden_states), \
                    (second_last_src_hidden_states, second_last_dst_hidden_states)
        else:
