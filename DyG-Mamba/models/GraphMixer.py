...
from models.modules import TimeEncoder, EdgeEncoder
...
class GraphMixer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 time_feat_dim: int, num_tokens: int, experiment_settings: ExperimentSetting=None, num_layers: int = 2, token_dim_expansion_factor: float = 0.5,
                 channel_dim_expansion_factor: float = 4.0, dropout: float = 0.1, device: str = 'cpu', 
                 use_mlp_instead: bool = False):
        """
        TCL model.
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        src_node_embeddings = self.compute_node_temporal_embeddings(node_ids=src_node_ids, node_interact_times=node_interact_times,
                                                                    num_neighbors=num_neighbors, time_gap=time_gap, edge_ids=edge_ids, 
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
                                         num_neighbors: int = 20, time_gap: int = 2000, edge_ids=None, robust_rate=0):
        """
        ...
        neighbor_node_ids, neighbor_edge_ids, neighbor_times = \
            self.neighbor_sampler.get_historical_neighbors(node_ids=node_ids,
                                                           ...
                                                           num_neighbors=num_neighbors, robust_rate=robust_rate)
        ...
        # nodes_edge_raw_features = (self.edge_raw_features[torch.from_numpy(edge_ids[:, np.newaxis])] - nodes_edge_raw_features).float().to(self.device)
        # nodes_edge_raw_features += self.edge_encoder(torch.from_numpy(edge_ids[:, np.newaxis] - neighbor_edge_ids).float().to(self.device))
        # nodes_edge_raw_features = self.edge_encoder(torch.from_numpy(edge_ids[:, np.newaxis]).float().to(self.device)) * self.edge_encoder(torch.from_numpy(neighbor_edge_ids).float().to(self.device))
        # Tensor, shape (batch_size, num_neighbors, time_feat_dim)
        # nodes_neighbor_time_features = self.time_encoder(timestamps=torch.from_numpy(edge_ids[:, np.newaxis] - neighbor_edge_ids).float().to(self.device))
        ...
        time_gap_neighbor_node_ids, _, _ = self.neighbor_sampler.get_historical_neighbors(node_ids=node_ids,
                                                                                          node_interact_times=node_interact_times,
                                                                                          num_neighbors=time_gap, robust_rate=robust_rate)
...
class MLPEncoder(nn.Module):
    ...
    def __init__(self, num_tokens: int, num_channels: int, token_dim_expansion_factor: float = 0.5,
        ...
        self.token_norm = nn.LayerNorm(num_tokens)
        self.channel_feedforward1 = FeedForwardNet(input_dim=num_channels, dim_expansion_factor=channel_dim_expansion_factor,
                                                dropout=dropout)

        self.channel_norm = nn.LayerNorm(num_channels)
        self.channel_feedforward2 = FeedForwardNet(input_dim=num_channels, dim_expansion_factor=channel_dim_expansion_factor,
                                                  dropout=dropout)
    ...
    def forward(self, input_tensor: torch.Tensor):
        ...
        # mix tokens
        # Tensor, shape (batch_size, num_channels, num_tokens)
        hidden_tensor = self.token_norm(input_tensor.permute(0, 2, 1)).permute(0, 2, 1)
        # Tensor, shape (batch_size, num_tokens, num_channels)
        hidden_tensor = self.channel_feedforward1(hidden_tensor)
        # Tensor, shape (batch_size, num_tokens, num_channels), residual connection
        output_tensor = hidden_tensor + input_tensor
        ...
        hidden_tensor = self.channel_norm(output_tensor)
        # Tensor, shape (batch_size, num_tokens, num_channels)
        hidden_tensor = self.channel_feedforward2(hidden_tensor)
        # Tensor, shape (batch_size, num_tokens, num_channels), residual connection
        output_tensor = hidden_tensor + output_tensor
