...
class PositionEncoder(nn.Module):
    ...
    def count_nodes_appearances(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        # src_nodes_neighbor_ids: [hop1: array(batch_size, num_neighbors), hop2: array(batch_size, num_neighbors**2), ...], 邻居中可能有pad id 0，但是后面不会对0节点计数。
        ...
        for idx, (src_node_id, dst_node_id, node_interact_time) in enumerate(zip(src_node_ids, dst_node_ids, node_interact_times)):
            ...
            src_node_neighbor_ids = [src_nodes_single_hop_neighbor_ids[idx] for src_nodes_single_hop_neighbor_ids in src_nodes_neighbor_ids] # [num_neighbors, num_neighbors**2, ...]
            ...
            for current_hop in range(self.walk_length + 1):
                for src_node_neighbor_id, src_node_neighbor_time, dst_node_neighbor_id, dst_node_neighbor_time in \
                    ...
                    src_node_key = '-'.join([str(idx), str(src_node_neighbor_id)]) # 0-5, 0-7, ..., next hop, 0-21
                    ...
                    tmp_nodes_appearances[src_node_key][0, current_hop] += 1 / num_current_hop_neighbors # 0表示该node_key在src对应序列的出现次数
                    tmp_nodes_appearances[dst_node_key][1, current_hop] += 1 / num_current_hop_neighbors # 1表示该node_key在dst对应序列的出现次数
    ...
    def forward(self, nodes_neighbor_ids: np.ndarray):
        ...
        node_appearances = unique_node_appearances[inverse_indices, :].reshape(nodes_neighbor_ids.shape[0], nodes_neighbor_ids.shape[1],
                                                                               nodes_neighbor_ids.shape[2], 2, self.walk_length + 1) # 这一步将节点id变为节点出现频次
        ...
        position_features = self.position_encode_layer(torch.Tensor(node_appearances).float().to(self.device)) # 可以看作是各个hop间的加权求和
        ...
        # Tensor, shape (batch_size, num_neighbors ** self.walk_length, self.walk_length + 1, position_feat_dim)
        position_features = position_features.sum(dim=-2) # 可以看作是各节点的出现频次在src和dst方向上求和
