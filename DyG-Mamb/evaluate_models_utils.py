...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   ...
                                   num_neighbors: int = 20, time_gap: int = 2000, is_evaluation: bool=False, logger=None, robust_rate=0):
    """
    ...
    if model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer', 'DyGMamba', 'FreeDyG']:
    ...
    with torch.no_grad():
        ...
        evaluate_idx_data_loader_tqdm = tqdm(evaluate_idx_data_loader, ncols=120, total=len(evaluate_idx_data_loader), miniters=20, mininterval=1.0)
            ...
            if model_name in ['TGAT', 'CAWN', 'TCL']:
                ...
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                                                                      ...
                                                                      num_neighbors=num_neighbors,
                                                                      time_gap=time_gap,
                                                                      edge_ids=batch_edge_ids, robust_rate=robust_rate)
                ...
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                                                                        dst_node_ids=batch_dst_node_ids,
                                                                        node_interact_times=batch_node_interact_times, robust_rate=robust_rate
                                                                        )
                ...
                batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                                                                        ...
                                                                        node_interact_times=batch_node_interact_times,
                                                                        edge_ids=None,
                                                                        edges_are_positive=False, is_evaluation=is_evaluation, robust_rate=robust_rate)
                ...
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                                                                        ...
                                                                        node_interact_times=batch_node_interact_times,
                                                                        edge_ids=batch_edge_ids,
                                                                        edges_are_positive=True, is_evaluation=is_evaluation, robust_rate=robust_rate)
