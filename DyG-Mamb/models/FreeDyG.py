...
class FreeDyG(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        self.projection_layer = nn.ModuleDict({
            ...
            'nif': nn.Linear(in_features=self.nif_feat_dim, out_features=self.node_feat_dim, bias=True)
        })
        self.reduce_layer = nn.Linear(4*self.node_feat_dim, self.node_feat_dim)
       
        self.mlp_mixers = nn.ModuleList([
            MLPMixer(num_tokens=max_input_sequence_length, num_channels=self.node_feat_dim,
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray, robust_rate=0):
        ...
        src_nodes_neighbor_ids, src_nodes_edge_ids, src_nodes_neighbor_times = \
           self.neighbor_sampler.get_historical_neighbors(node_ids=src_node_ids,
                                                           ...
                                                           num_neighbors=self.max_input_sequence_length, robust_rate=robust_rate)
