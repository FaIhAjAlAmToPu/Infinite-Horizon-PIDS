...
class MemoryModel(torch.nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 time_feat_dim: int, experiment_settings: ExperimentSetting=None, model_name: str = 'TGN', num_layers: int = 2, num_heads: int = 2, dropout: float = 0.1,
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray,
                                                 edge_ids: np.ndarray, edges_are_positive: bool = True, num_neighbors: int = 20, robust_rate=0):
        """
...
class MemoryBank(nn.Module):
    ...
    def __init__(self, num_nodes: int, memory_dim: int):
        ...
        self.node_last_updated_times = nn.Parameter(torch.zeros(self.num_nodes), requires_grad=False) # 上一次的信息
        # dictionary of list, {node_id: list of tuples}, each tuple is (message, time) with type (Tensor shape (message_dim, ), a scalar)
...
class TimeProjectionEmbedding(nn.Module):
    ...
    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor, node_ids: np.ndarray, node_time_intervals: torch.Tensor, robust_rate=0):
        """
...
class GraphAttentionEmbedding(nn.Module):
    ...
    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor, node_ids: np.ndarray, node_interact_times: np.ndarray,
                                         current_layer_num: int, num_neighbors: int = 20, robust_rate=0):
        """
