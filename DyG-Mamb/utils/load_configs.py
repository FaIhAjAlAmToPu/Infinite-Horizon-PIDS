...
def get_link_prediction_args(is_evaluation: bool = False, args_list=None):
    """
    ...
    parser.add_argument('--model_name', type=str, default='DyGFormer', help='name of the model, note that EdgeBank is only applicable for evaluation',
                        choices=['JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'EdgeBank', 'TCL', 'GraphMixer', 'DyGFormer', 'DyGMamba', 'FreeDyG'])
    parser.add_argument('--gpu', type=int, default=0, help='number of gpu to use')
    parser.add_argument('--num_neighbors', type=int, default=20, help='number of neighbors to sample for each node') # for GraphMixer and TCL
    ...
    parser.add_argument('--comments', type=str, default=None, help='comments is the noticification to save model')
    parser.add_argument('--d_state', type=int, default=16, help='mamba state dimension')
    parser.add_argument('--d_conv', type=int, default=4, help='mamba conv1d kernel size')
    parser.add_argument('--expand', type=int, default=2, help='mamba hidden states expand')
    
    parser.add_argument('--time_consumption_test', type=str_to_bool, default=False, help='whether to print run time of each epoch')
    ...
    if args.model_name == 'DyGMamba':
        load_link_prediction_mamba(args=args, is_evaluation=is_evaluation)
    elif args.load_best_configs:
        load_link_prediction_best_configs(args=args)

    return args


def load_link_prediction_mamba(args: argparse.Namespace, is_evaluation=False):
    args.num_layers = 2 # 4
    args.batch_size = 200 # 20
    args.max_input_sequence_length = 1024 # 1024
    args.num_epochs = 200
        
    if args.dataset_name in ['reddit']:
        args.batch_size = 200
        args.max_input_sequence_length = 64 #64
        args.num_epochs = 400
        args.patience = 40
        if is_evaluation:
            args.batch_size = 600
    elif args.dataset_name in ['enron']:
        args.max_input_sequence_length = 256 # 256
        args.batch_size = 30 # 30
        args.num_epochs = 200
        args.patience = 40
        if is_evaluation:
            args.batch_size = 200
    elif args.dataset_name in ['SocialEvo']:
        args.max_input_sequence_length = 64 # 32
        args.batch_size = 200
        args.num_epochs = 200
        args.patience = 40
    elif args.dataset_name in ['USLegis']:
        args.max_input_sequence_length = 272 # 272
        args.batch_size = 20 # 200
        args.patch_size = 8
        args.num_epochs = 200 # 500
        args.patience = 40 # 40
        if is_evaluation:
            args.batch_size = 40
        ...
        args.max_input_sequence_length = 32 # 32
        args.batch_size = 200
        args.num_epochs = 500
        args.patience = 40
    elif args.dataset_name in ['lastfm']:
        args.max_input_sequence_length = 512 # 512
        args.patch_size = 16
        args.batch_size = 100
        args.num_epochs = 400
        args.patience = 40
    elif args.dataset_name in ['CanParl']:
        args.max_input_sequence_length = 2048 # 2048
        args.batch_size = 30
        if is_evaluation:
            args.batch_size = 150
    elif args.dataset_name in ['mooc']:
        args.max_input_sequence_length = 256 # 2048
        args.batch_size = 200
    elif args.dataset_name in ['UNvote']:
        args.max_input_sequence_length = 128 # 128
        args.batch_size = 200
        args.patch_size = 4
        args.num_epochs = 400
        args.patience = 40
...
def load_link_prediction_best_configs(args: argparse.Namespace):
    ...
    if args.model_name == 'TGAT':
        ...
        if args.dataset_name in ['reddit']:
            args.max_input_sequence_length = 64
            args.patch_size = 2
        elif args.dataset_name in ['mooc', 'Flights', 'USLegis', 'UNtrade']:
            ...
            args.max_input_sequence_length = 1024 # 2048
            args.patch_size = 64
        elif args.dataset_name in ['UNvote']:
            args.max_input_sequence_length = 128
            args.patch_size = 4
...
def get_node_classification_args():
    ...
    parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='wikipedia', choices=['wikipedia', 'reddit', 'mooc'])
    parser.add_argument('--batch_size', type=int, default=200, help='batch size')
    ...
    parser.add_argument('--d_conv', type=int, default=4, help='mamba conv1d kernel size')
    parser.add_argument('--expand', type=int, default=2, help='mamba hidden states expand')
    parser.add_argument('--delta_t', type=float, default=1, help='the time span of each time slot, used when equal_time_slot is True')
    ...
    assert args.dataset_name in ['wikipedia', 'reddit', 'mooc'], f'Wrong value for dataset_name {args.dataset_name}!'
    if args.model_name == 'DyGMamba':
        load_node_classification_mamba(args=args)
    elif args.load_best_configs:
...
def load_node_classification_mamba(args: argparse.Namespace, is_evaluation=False):
    args.num_layers = 2 # 4
    args.batch_size = 200 # 20
    args.max_input_sequence_length = 1024 # 1024
    args.num_runs = 5
