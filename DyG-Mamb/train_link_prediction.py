...
from utils.load_configs import get_link_prediction_args, ExperimentSetting
...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        args.save_model_name = f'{args.model_name}_seed{args.seed}' \
            if args.comments == None else f'{args.model_name}_seed{args.seed}_{args.comments}'
        ...
        if args.model_name == 'TGAT':
            ...
            config = MambaConfig()
            config.ssm_cfg = {"d_state": args.d_state, "d_conv": args.d_conv, "expand": args.expand} # "d_state": 16, "d_conv": 4, "expand": 2
            dynamic_backbone = DyGMamba(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=train_neighbor_sampler,
                                         time_feat_dim=args.time_feat_dim, config=config, channel_embedding_dim=args.channel_embedding_dim, 
                                         patch_size=args.patch_size, num_layers=args.num_layers, dropout=args.dropout, 
                                         max_input_sequence_length=args.max_input_sequence_length, device=args.device, 
                                         experiment_settings=None, logger=logger)
        else:
        ...
        for epoch in range(args.num_epochs):
            ...
            model.train()
            if args.model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer', 'DyGMamba', 'FreeDyG']:
            ...
            epoch_start_time = 0
            if args.time_consumption_test == True:
                epoch_start_time = time.time()
            if args.model_name in ['DyGMamba']:
                mininterval = 20.0
                maxinterval = 40.0
                if args.dataset_name in ['reddit', 'Flights', 'SocialEvo', 'Contacts', 'lastfm']:
                    mininterval = 60.0
                    maxinterval = 100.0
            else:
                mininterval = 2.0
                maxinterval = 10.0
            train_idx_data_loader_tqdm = tqdm(train_idx_data_loader, ncols=120, total=len(train_idx_data_loader), miniters=40, 
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                # if args.model_name in ['FreeDyG']:
                #     logger.info(f'positive_probabilities: {positive_probabilities.shape}, negative_probabilities: {negative_probabilities.shape}')
            ...
            if args.time_consumption_test == True:
                ...
                logger.info(f"train epoch run time: {epoch_end_time - epoch_start_time}")
        ...
        if not (args.model_name in ['JODIE', 'DyRep', 'TGN']):
            val_losses, val_metrics = evaluate_model_link_prediction(model_name=args.model_name,
                                                                     model=model,
                                                                     neighbor_sampler=full_neighbor_sampler,
                                                                     evaluate_idx_data_loader=val_idx_data_loader,
    ...
    with open('all_results.txt', 'a') as file:
        file.write(f'{args.dataset_name} {args.batch_size} {args.max_input_sequence_length} {args.num_layers} {args.num_runs} {args.channel_embedding_dim} start\n')

        for metric_name in test_metric_all_runs[0].keys():
            file.write(f'test {metric_name}, {[test_metric_single_run[metric_name] for test_metric_single_run in test_metric_all_runs]}\n')
            file.write(f'average test {metric_name}, {np.mean([test_metric_single_run[metric_name] for test_metric_single_run in test_metric_all_runs]):.4f} '
                        f'± {np.std([test_metric_single_run[metric_name] for test_metric_single_run in test_metric_all_runs], ddof=1):.4f}\n')

        for metric_name in new_node_test_metric_all_runs[0].keys():
            file.write(f'new node test {metric_name}, {[new_node_test_metric_single_run[metric_name] for new_node_test_metric_single_run in new_node_test_metric_all_runs]}\n')
            file.write(f'average new node test {metric_name}, {np.mean([new_node_test_metric_single_run[metric_name] for new_node_test_metric_single_run in new_node_test_metric_all_runs]):.4f} '
                        f'± {np.std([new_node_test_metric_single_run[metric_name] for new_node_test_metric_single_run in new_node_test_metric_all_runs], ddof=1):.4f}\n')
